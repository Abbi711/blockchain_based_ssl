...
def revoke():
    ...
    fh=open("revoke_list.txt","a") 
    fh.write(version)
    fh.write("\n")
    fh.close()
    fh1=open("details.txt","a") 
    str1="The certificate "+version+" has been revoked"
    fh1.write(str1)
    fh1.write("\n")
    fh1.close()
    return redirect('/revoke1')
...
def validate():
    ...
    class BloomFilter:
        def __init__(self, list1):       	
               ...
               for word in list1:
                   for seed in range(self.seed_count):
                       t=(word,seed)
                       hashIndex = hash(t) % self.array_length
        ...
        def probable_member(self, word):
            for seed in range(self.seed_count):
                t=(word,seed)
                candidateHash = hash(t) % self.array_length
        ...
        def lookup(self, word):
            if self.probable_member(word):
               return '"{}" is most likely in the revoked list'.format(word)
            else:
               return '"{}" is not in the revoked list'.format(word)


    
    list1=[]
    list1 = [line.rstrip('\n') for line in open('revoke_list.txt', 'r')]
    bloom_filter = BloomFilter(list1)
    str11= bloom_filter.lookup(version)
...
def new_transaction():
    ...
    tx_data = post_object
    required_fields = ["version", "serial", "signature", "issuer", "validity", "subject", "public_key", "issuer_id", "subject_id", "extension"]
    ...
    data['cert'].append({
        ...
        'serial': tx_data.get('serial'),
	'signature': tx_data.get('signature'),
	'issuer': tx_data.get('issuer'),
	'validity': tx_data.get('validity'),
	'public_key': tx_data.get('public_key'),
	'issuer_id': tx_data.get('issuer_id'),
	'subject_id': tx_data.get('subject_id'),
	'extension': tx_data.get('extension'), 
	})
...
def register_new_peers():
    ...
    #node_address = request.form["version"]
    if not node_address:
...
def get_pending_tx():
    if len(blockchain.unconfirmed_transactions)==0:
        return "No unconfirmed transactions"
